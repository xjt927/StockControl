...
namespace Stock.Tools.Utility
{
    ...
    public static class EnumHelper
    {
        ...
        /// <returns></returns>
        public static string GetEnumName<T>(long value) where T : new()
        {
            Type t = typeof(T);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static string GetDefaultValue<T>(long value) where T : new()
        {
            Type t = typeof(T);
            foreach (MemberInfo mInfo in t.GetMembers())
            {
                if (mInfo.Name == t.GetEnumName(value))
                {
                    foreach (Attribute attr in Attribute.GetCustomAttributes(mInfo))
                    {
                        ...
                        {
                            return ((DefaultValueAttribute)attr).Value.ToString();
                        }
                    }
                }
            }
        ...
        /// </summary> 
        /// <returns></returns>
        public static string GetDefaultValue(this Enum eEnum)
        {
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static IEnumerable<Tuple<string, string>> GetEnumNamesValues<T>() where T : new()
        {
            Type t = typeof(T);
            //获取 枚举 值
            Array valueArray = Enum.GetValues(t);
            ...
            {
                var obj = fieldinfo.ToList().Find(item => item.Name == value.ToString()).GetCustomAttributes(typeof(DescriptionAttribute), false);

                if (obj.Length == 0) continue;
                var des = (DescriptionAttribute)obj[0];

                tuple.Add(new Tuple<string, string>(((long)value).ToString(CultureInfo.InvariantCulture),
                 des.Description));
            }
            return tuple;
        }
        ...
        /// 获取枚举Enum中的描述和值
        /// </summary>
        public static List<Tuple<long, string>> GetEnumItems<T>() where T : new()
        {
            return (from object item in Enum.GetValues(typeof(T))
                    select
                        new Tuple<long, string>((long)item, GetEnumName<T>((long)item))).ToList();
        }
